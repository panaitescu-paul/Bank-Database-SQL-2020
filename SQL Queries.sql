...
CREATE TABLE Branch (
id INT,
name CHAR(50) UNIQUE, address CHAR(50), PRIMARY KEY (id)
); 
...
CREATE TABLE Card (
id INT,
number CHAR(50) UNIQUE, expiration_date DATE, is_blocked BOOL, PRIMARY KEY (id)
 ); 
...
id INT,
type CHAR(10) UNIQUE, description CHAR(100), base_amount DECIMAL, base_interest_rate DECIMAL, PRIMARY KEY (id)
); 
...
id INT,
type CHAR(50) UNIQUE,
description CHAR(100), base_amount DECIMAL(10, 3), base_interest_rate DECIMAL(10, 3), PRIMARY KEY (id)
); 
...
id INT, customer_id INT, card_id INT,
 balance CHAR(50),
PRIMARY KEY (id),
...
CREATE USER 'paul2'@'%' IDENTIFIED BY 'password'; CREATE USER 'constantin2'@'%' IDENTIFIED BY 'password'; CREATE USER 'marius2'@'%' IDENTIFIED BY 'password'; GRANT ALL ON *.* TO 'paul2'@'%';
GRANT ALL ON *.* TO 'constantin2'@'%' WITH GRANT OPTION; GRANT SELECT, UPDATE, DELETE ON *.* TO 'marius2'@'%'; SELECT * FROM mysql.user;
...
-- ------------------------------------------------
USE Bank;
INSERT INTO Card (id, number, expiration_date,
...
INSERT INTO Card (id, number, expiration_date, is_blocked) ('5', '1234567890123450', '2021-06-9', TRUE); 
...
VALUES
USE Bank;
INSERT INTO Loan_type (id,
base_interest_rate) VALUES
10000, 15);
INSERT INTO Loan_type (id,
base_interest_rate) VALUES
20);
...
is_blocked) is_blocked) is_blocked) is_blocked)
type, description, base_amount,
('1', 'Mortgages loans', 'description1',
type, description, base_amount,
('2', 'Car loans', 'description2', 5000,
type, description, base_amount,
('3', 'Appliance loans', 'description3',
type, description, base_amount, ('4', 'Payday loans', 'description4',
type, description, base_amount, ('5', 'Small Business loans',
...
USE Bank;
INSERT INTO Transaction (id, account_id, description, amount, date) VALUES ('1', '1', 'description 100', '1000.90', '2020-05-18'); INSERT INTO Transaction (id, account_id, description, amount, date) VALUES ('2', '2', 'description 200', '500.80', '2019-12-07'); INSERT INTO Transaction (id, account_id, description, amount, date) VALUES ('3', '5', 'description 300', '100.90', '2018-06-30');
(id, customer_id, card_id, balance) VALUES ('1', (id, customer_id, card_id, balance) VALUES ('2', (id, customer_id, card_id, balance) VALUES ('3', (id, customer_id, card_id, balance) VALUES ('4', (id, customer_id, card_id, balance) VALUES ('5',

 INSERT INTO Transaction (id, account_id, description, amount, date) VALUES ('4', '5', 'description 400', '5060.7', '2020-01-24'); INSERT INTO Transaction (id, account_id, description, amount, date) VALUES ('5', '5', 'description 500', '500.67', '2018-01-24');
...
BEGIN
DECLARE message varchar(50); IF NEW.balance < 100 THEN
SET message= CONCAT('Insertion error: new balance too low: ', NEW.balance);
SIGNAL SQLSTATE '46000' SET MESSAGE_TEXT = message;
END IF; END;
//
CREATE TRIGGER bal_limit_update BEFORE UPDATE ON Account FOR EACH ROW BEGIN
DECLARE message varchar(50); IF NEW.balance < 100 THEN
...
WHERE c.id IN (SELECT customer_id FROM Customer_Branch cb
...
USE Bank;
SELECT gender, COUNT(*) AS count FROM Customer AS c
WHERE c.id IN (
SELECT customer_id FROM Account AS a WHERE a.id IN (
SELECT account_id
FROM Loan AS l)) GROUP BY gender
ORDER BY count DESC; 
...
ON SCHEDULE AT '2020-12-31' + INTERVAL 1 year DO SELECT *
FROM Transaction t
...
FROM Account cb GROUP BY customer_id);
